...
void Sprite::InitConstantBuffer()
{
	...
	// 定数バッファの作成
	Direct3D::GetInstance().Device()->CreateBuffer(&cb, NULL, &pConstantBuffer_);
}
...
void Sprite::InitVertex()
{
	...
	Direct3D::GetInstance().Device()->CreateBuffer(&bd_vertex, &data_vertex, &pVertexBuffer_);
}
...
void Sprite::InitIndex()
{
	...
	Direct3D::GetInstance().Device()->CreateBuffer(&bd, &InitData, &pIndexBuffer_);
}
...
void Sprite::Draw(Transform& transform, RECT rect, float alpha)
{
	...
	d3D->Context()->VSSetConstantBuffers(0, 1, &pConstantBuffer_);
	d3D->Context()->PSSetConstantBuffers(0, 1, &pConstantBuffer_);
	d3D->SetDepthBafferWriteEnable(false);
	...
	XMMATRIX view = XMMatrixScaling(1.0f / wm->GetWindow("Editor")->Width(), 1.0f / wm->GetWindow("Editor")->Height(), 1.0f);

	//最終的な行列
	XMMATRIX world = cut * view * transform.WoaldMatrix();
	...
	d3D->Context()->Map(pConstantBuffer_, 0, D3D11_MAP_WRITE_DISCARD, 0, &pdata);	// GPUからのリソースアクセスを一時止める
	...
	d3D->Context()->Unmap(pConstantBuffer_, 0);									// GPUからのリソースアクセスを再開
	...
	d3D->SetDepthBafferWriteEnable(true);
}
