...
bool Model::Load(string _fileName)
{
	//assimpLoaderで情報を取得
	AssimpLoader al;
	if (al.Load(_fileName, meshes_, false, true) == false)return false;
...
void Model::InitVertex(const Mesh& _mesh)
{
    Direct3D& d3d = Direct3D::GetInstance();
    vertices_ = _mesh.vertices;
...
void Model::InitIndex(const Mesh& _mesh)
{
    Direct3D& d3d = Direct3D::GetInstance();
    indices_ = _mesh.indices;
